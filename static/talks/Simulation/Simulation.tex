...
\CommentTok{\# summary(compare)[,"mean"]}
\end{Highlighting}
\end{Shaded}
...
\hline
\rowcolor{gray!6}  NN & 21 & 0.983 & 0.873 & 0.010 & 0.005 & 0.476 & 5.321\\
\hline
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Plotting using optimal parameters}
\NormalTok{eps\_optim \textless{}{-}}\StringTok{ }\NormalTok{quality\_eps[}\KeywordTok{which.max}\NormalTok{(}\KeywordTok{as.tibble}\NormalTok{(quality\_eps[}\OperatorTok{{-}}\DecValTok{1}\NormalTok{,])}\OperatorTok{$}\NormalTok{lcmc), }\StringTok{"eps"}\NormalTok{]}
\NormalTok{sr\_isomap\_ann\_optim \textless{}{-}}\StringTok{ }\KeywordTok{embed}\NormalTok{(sr, }\StringTok{"annIsomap"}\NormalTok{, }\DataTypeTok{knn =}\NormalTok{ K, }\DataTypeTok{eps =}\NormalTok{ eps\_optim, }\DataTypeTok{ndim =} \DecValTok{2}\NormalTok{, }\DataTypeTok{get\_geod =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{.mute =} \KeywordTok{c}\NormalTok{(}\StringTok{"message"}\NormalTok{, }\StringTok{"output"}\NormalTok{))}

\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(sr, }\DataTypeTok{type =} \StringTok{"2vars"}\NormalTok{, }\DataTypeTok{main =} \StringTok{"Swissroll data"}\NormalTok{)}
...
\KeywordTok{plot}\NormalTok{(sr\_isomap\_ann\_optim, }\DataTypeTok{type =} \StringTok{"2vars"}\NormalTok{, }\DataTypeTok{main =} \KeywordTok{paste}\NormalTok{(}\StringTok{"ANN with K="}\NormalTok{, K, }\StringTok{"eps="}\NormalTok{, eps\_optim))}
\end{Highlighting}
\end{Shaded}

...
- Statistical manifold: manifold with each observation as PDFs
...
.column.center[
```{r isoembed, fig.align = 'right', out.width = "70%", fig.cap="", echo = FALSE, message=FALSE}
# knitr::include_graphics("images/isoembed.png")
```
]
]
]
...
Annoy and HNSW are brought up in the recent 5 year. And they both work better than k-d trees. [ann-benchmark.com]
...
with higher value indicating a higher-quality representation
...
- World map distances between two locations are not the geometric distances.
...
- In Chapter 1, the HDR plots use KDE with single $H$ matrix
...
<!-- ??? -->
<!-- If the inner product of the tangent space [if exists] is known for a given geometry, the Riemannian metric is a good measure to recover the geometry of manifold. -->
...
*Github repo*: [@ffancheng](https://github.com/ffancheng/paper-mlann)
